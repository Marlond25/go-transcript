...
func main() {
	...
	// Output and other methods of Command will return *exec.Error if ther was a problem executing the command (e.g. wrong path), and *exec.ExitError if the command ran but exited with a non-zero return code.
	...
	// Note that when spawning commands we need to provide an explicitly delineated command and argument array, vs. being able to just pass in one command with a string, you can use bash's -c option:
