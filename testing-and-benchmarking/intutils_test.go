...
// We'll be testing this simple implementation of an integer minimun. Typically, the code we're testing would be in a source file named something like intutils.go, and the test file for it would then be named intutils_test.go.
...
// Writing test can be repetitive, so it's idiomatic to use a table-driven style, where test inputs and expected outputs are listed in a table and a single loop walks over them and performs the test logic.
